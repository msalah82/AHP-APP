...
# Load session metadata (text inputs) BEFORE widgets
criteria_input = st.session_state.get("session_metadata_criteria", "Cost, Quality, Time")
alternative_input = st.session_state.get("session_metadata_alternatives", "Option A, Option B, Option C")

# Input widgets initialized from session-safe values
criteria_input = st.text_input("Enter criteria (comma-separated)", criteria_input, key="criteria_input")
alternative_input = st.text_input("Enter alternatives (comma-separated)", alternative_input, key="alternative_input")

# Parse input into usable lists
...
if st.sidebar.button("🔄 Save Session"):
    session_data = {k: v for k, v in st.session_state.items() if k not in ["criteria_input", "alternative_input"] and not k.startswith("_")}
...
if key in st.session_state:
                val = st.slider(label, 1/9.0, 9.0, step=0.01, format="%.2f", key=key)
            else:
                val = st.slider(label, 1/9.0, 9.0, 1.00, step=0.01, format="%.2f", key=key)
            val = st.slider(label, 1/9.0, 9.0,
                            st.session_state[session_key].get(label, 1.00),
