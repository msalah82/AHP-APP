...
st.title("✅ Final AHP Tool with 2 Decimal Precision")
...
criteria = st.text_input("Enter criteria (comma-separated)", "Cost, Quality, Time").split(",")
criteria = [c.strip() for c in criteria if c.strip()]
alternatives = st.text_input("Enter alternatives (comma-separated)", "Option A, Option B, Option C").split(",")
...
if len(criteria) >= 2 and len(alternatives) >= 2:
    ...
    if st.button("📝 Export to Word"):
        doc = Document()
        doc.add_heading("AHP Decision Report", 0)
