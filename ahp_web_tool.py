...
# Prevent session overwrite after rerun
if "session_loaded" not in st.session_state:
    st.session_state.session_loaded = False

if not st.session_state.session_loaded:
    criteria = st.text_input("Enter criteria (comma-separated)", "Cost, Quality, Time", key="criteria_input").split(",")
    criteria = [c.strip() for c in criteria if c.strip()]
    alternatives = st.text_input("Enter alternatives (comma-separated)", "Option A, Option B, Option C", key="alternative_input").split(",")
